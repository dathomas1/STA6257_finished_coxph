...
tbl_survfit(kmfit, 
  ...
  modify_caption("**Table 6: Survival Probability for Particular Days for Treatment Only Model**") 
```
...
add_glance_table(
  ...
  modify_caption("**Table : Events after 180 days Model**") 
```
...
anova(cox.mod180.r, cox.mod180, test="LRT")
cox.mod1 = cox.mod180
```
...
$$h(t) = {h_0(t)} \times \exp(-0.317 \text{(treat_test)}+0.012 \text{(age)}-0.0087 \text{(Karn)}+0.112\text{(diag.time)}-0.502 \text{(cell_small)}+0.965 \text{(cell_large)}-0.642 \text{(prior_no)})$$
...
add_glance_table(
  ...
  modify_caption("**Table : Stratified Cox Model**") 
```
...
$$h_g(t) = {h_{0g}(t)} \times \exp(0.286\text{(treat_test)}-0.012\text{(age)}-0.0383\text{(Karn)}-0.0034\text{(diag.time)}+0.169\text{(prior_no)})$$
where $$g = 1, 2, 3, 4 \text{ (# of cell types)}$$ 

```{r}
cox.mod2 = cox.modstrat
cox.zph(cox.modstrat, transform="rank")
```
...
```{r}
plot(survfit(cox.modstrat), lty = c("solid","dashed", "dotted","longdash"), col=c("black","grey","blue","orange"),
xlab="survival time",ylab="survival probabilities")
...
add_glance_table(
  ...
  modify_caption("**Table : Extended Cox Model**") 
```

```{r}
cox.zph(cox.modextend, transform="rank")
```
...
cox.zph(cox.modse, transform="rank")
```
...
add_glance_table(
  ...
  modify_caption("**Table : Stratified-Extended Cox Model**") 
```


$$h_g(t) = {h_{0g}(t)} \times \exp(0.0988\text{(treat_test)}-0.0464\text{(Karn*tgroup_1)}+0.0097\text{(Karn*tgroup_2)}-0.0179\text{(Karn*tgroup_3)}$$
where $$g = 1, 2, 3, 4 \text{ (# of cell types)}$$

and tgroup_1 = 1 if $ 0 \le t < 90 $,
 tgroup_2 = 1 if $ 90 \le t < 180 $,
 tgroup_3 = 1 if $ t \ge $
