...
cox.unimod <- coxph(Surv(stime, status) ~ treat, data=VA)
...
plot(cox.zph(cox.unimod))
```

```{r}
# Plotting graph of Kaplan-Meier curve
# Notice how Test treatment is less effective in the beginning but better at the end.
kmfit = survfit(Surv(stime, status) ~ treat, data=VA)
...
Now, we will use all the variables in the multiple Cox PH model.
```{r}
