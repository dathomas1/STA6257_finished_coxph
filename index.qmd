...
tbl_survfit(kmfit, 
  ...
  modify_caption("**Table 6: Treatment Only Model**") 
```
...
add_glance_table(
  ...
  modify_caption("**Table 7: Treatment Only Model**") 
```
...
```{r}
cox.mod1 <- coxph(Y ~ treat + age + Karn + diag.time + strata(cell) + prior, data=VA)
summary(cox.mod1)
cox.zph(cox.mod1, transform="rank")
```


#### Extended Cox PH model for Time-Dependent
It would be unreliable for us to use the Cox PH across the entire time interval. For this analysis, we will run the Cox PH model over specific time intervals where the hazard ratio is constant. Before we can run the Cox PH across the intervals of 0-90, 90-180, and 180+ days, we must create a new column from the original dataset.

```{r}
VA.cp = survSplit(Surv(stime, status)  ~ ., data=VA, cut=c(90,180), episode="tgroup")

#look at first few rows
kable(head(VA.cp, 5), caption = "Table : Sample Rows with new column") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = TRUE) %>%
  row_spec(0, bold = T, background = "#666666", color = "white")
```
...
```{r}
cox.mod2 = coxph(Surv(tstart, stime, status) ~ treat + age + diag.time + cell + prior + Karn:strata(tgroup),
                 data=VA.cp)

summary(cox.mod2)
cox.zph(cox.mod2, transform="rank")
```

```{r}
cox.mod3 = coxph(Surv(tstart, stime, status) ~ treat + age + diag.time + strata(cell) + prior + Karn:strata(tgroup),
                 data=VA.cp)

summary(cox.mod3)
cox.zph(cox.mod3, transform="rank")
```


```{r}
cox.mod4 = coxph(Surv(tstart, stime, status) ~ treat + strata(cell) + Karn:strata(tgroup),
                 data=VA.cp)

summary(cox.mod4)
cox.zph(cox.mod4, transform="rank")
```
...
anova(cox.mod4, cox.mod3, test="LRT")
...
### 3.4. Interpretations [Need to revisit]
