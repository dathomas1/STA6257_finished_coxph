...
-   Karnofsky and treatment seem positively correlated
...
![](index_files/figure-html/unnamed-chunk-8-1.png){width="800"}
:::
...
```{r, echo=TRUE}
coxph(Surv(tstart, stime, status) ~ treat + age + diag.time + cell:strata(tgroup) + prior + Karn:strata(tgroup),
                 data=VA.cp)
```
:::
...
-   Reiterating why it's popular in survival analysis
